...
def apply_corrections(pdf: Path, corrections: list[dict], shapes_json: Path | None = None, min_iou: float = 0.3) -> list[dict]:
    ...
    for c in corrections:
        ...
        best = max(glyphs, key=lambda g: _iou(g["box_px"], c["box"]), default=None)
        score = _iou(best["box_px"], c["box"]) if best else 0.0
        if not best or score < min_iou:
    ...
    if shapes_json and Path(shapes_json).exists():
        ...
        for d in done:
            ...
            for p in s["placements"]:
                if p["page"] == d["page"] and _iou(p["box"], d["box"]) >= min_iou:
