...
for r in reps:
    ...
    if not pdf: continue
    c, n = coverage(pdf); v = r.get("verify") or {}; why.update(r.get("letters_why") or {})
    rows.append(dict(stem=stem, pages=len(r.get("pages", [])), cut=c, words=n, verify=v, letters=(r.get("letters") or {}).get("accepted", 0)))
rows.sort(key=lambda x: x["cut"] / max(1, x["words"]))
C = sum(x["cut"] for x in rows); N = sum(x["words"] for x in rows)
print(f"{len(rows)} documents, {sum(x['pages'] for x in rows)} pages")
